...
readonly NETWORK_TEST_COUNT="$(grep -RhE '^(async )?test "' src/server | wc -l | tr -d ' ')"
if [ "${NETWORK_TEST_COUNT}" -lt 58 ]; then
  echo "NETWORK acceptance suite has only ${NETWORK_TEST_COUNT} tests; expected at least 58" >&2
  exit 1
fi
...
for required_test in \
  ...
  if ! grep -RqsE "^(async )?test \"${required_test}\" \\{" src/server; then
    echo "NETWORK acceptance test is missing: ${required_test}" >&2
    exit 1
  fi
done
...
tests/integration/network_interop.sh
