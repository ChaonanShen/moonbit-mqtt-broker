...
readonly OUTPUT="${PERF_OUTPUT:-.local/p1-03a-execution/performance-$(date -u +%Y%m%dT%H%M%SZ)}"
mkdir -p "${OUTPUT}"
...
token='perfread.0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef'
digest=hashlib.sha256(('moonbit-mqtt-broker/admin/v1:'+token).encode()).hexdigest()
(p/'tokens').write_text('perfread:'+digest+':metrics,read\n')
(p/'tokens').chmod(0o600)
PY
readonly TOKEN='perfread.0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef'
run_one() {
  local mode="$1" round="$2"
  container="moonbit-mqtt-perf-${mode}-${round}-$$"
  local args=(--listen "127.0.0.1:${MQTT_PORT}")
  if [[ "${mode}" != base ]]; then
    args+=(--management-enabled true --management-listen "127.0.0.1:${ADMIN_PORT}"
      --management-token-file /secrets/tokens --management-max-connections 32
      --management-max-bytes-total 16777216)
  fi
  ...
  docker run --rm --platform linux/amd64 --network host \
    ...
    tests/integration/management_performance.mjs \
