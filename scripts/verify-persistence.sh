...
scripts/verify-qos1.sh
moon fmt --check
moon check --target native
moon test --target native
moon build --target native

readonly PERSISTENCE_TEST_COUNT="$(grep -RhE '^(async )?test "' src | wc -l | tr -d ' ')"
if [[ "${PERSISTENCE_TEST_COUNT}" -lt 186 ]]; then
  echo "PERSISTENCE acceptance suite has only ${PERSISTENCE_TEST_COUNT} tests; expected at least 186" >&2
  exit 1
fi
...
for required_test in \
  ...
  if ! grep -RqsE "^(async )?test \"${required_test}\" \{" src; then
    echo "PERSISTENCE acceptance test is missing: ${required_test}" >&2
    exit 1
  fi
done
...
for required_integration in \
  ...
  if ! grep -RqsF "${required_integration}" tests/integration/persistence_*.sh; then
    echo "PERSISTENCE integration gate is missing: ${required_integration}" >&2
    exit 1
  fi
done
...
tests/integration/persistence_restart.sh
tests/integration/persistence_crash.sh
tests/integration/persistence_interop.sh
...
echo "PERSISTENCE verification passed (${PERSISTENCE_TEST_COUNT} tests)"
