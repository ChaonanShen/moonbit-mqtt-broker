...
scripts/m2-verify.sh
moon fmt --check
moon check --target native

readonly M3_TEST_COUNT="$(grep -RhE '^(async )?test "' src | wc -l | tr -d ' ')"
if [[ "${M3_TEST_COUNT}" -lt 130 ]]; then
  echo "M3 acceptance suite has only ${M3_TEST_COUNT} tests; expected at least 130" >&2
  exit 1
fi
...
for required_test in \
  ...
  if ! grep -RqsE "^(async )?test \"${required_test}\" \{" src; then
    echo "M3 acceptance test is missing: ${required_test}" >&2
    exit 1
  fi
done
...
tests/integration/m3_interop.sh
