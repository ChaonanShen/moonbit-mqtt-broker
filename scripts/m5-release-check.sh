...
readonly PACKAGE="${M5_PACKAGE_PATH:-${REPO_ROOT}/_build/publish/ChaonanShen-moonbit-mqtt-broker-0.1.0.zip}"
...
if [[ -n "${M5_VERIFIER_RECORD:-}" ]]; then
  test -r "${M5_VERIFIER_RECORD}"
  [[ "$(grep -c '^PASS ' "${M5_VERIFIER_RECORD}")" -ge 3 ]]
  grep -q '^SOAK PASS ' "${M5_VERIFIER_RECORD}"
else
  echo 'M5_VERIFIER_RECORD is not set; verifier/soak evidence is required before publish' >&2
  exit 1
fi
...
echo "M5 release preflight passed for ${EXPECTED_SHA}; no external writes performed"
