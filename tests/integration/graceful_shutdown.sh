...
run_without_persistence() {
  ...
  mosquitto_pub -h 127.0.0.1 -p "${port}" -q 1 \
    -t shutdown/memory-only -m value
...
run_signal_case() {
  ...
  node tests/integration/graceful_shutdown.mjs seed \
    "mqtt://127.0.0.1:${port}" "${case_name}" >"${seed_log}" 2>&1 &
  SEED_PID="$!"
  for _ in $(seq 1 300); do
    if grep -q "SHUTDOWN_SEED_READY=${case_name}" "${seed_log}"; then
      break
    fi
  ...
  grep -q "SHUTDOWN_SEED_READY=${case_name}" "${seed_log}"
  ...
  : >"${broker_log}"
  start_broker "${port}" "${data_dir}" "${broker_log}"
  node tests/integration/graceful_shutdown.mjs verify \
...
run_signal_case INT
run_without_persistence
echo 'SHUTDOWN SIGTERM/SIGINT final snapshot and Will suppression passed'
