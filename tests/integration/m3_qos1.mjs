...
if (!url) throw new Error('usage: node m3_qos1.mjs mqtt://HOST:PORT')
...
const live = await connectClient('m3-mqttjs-live')
const publisher = await connectClient('m3-mqttjs-publisher')
const grants = await subscribe(live.client, 'm3/mqttjs/live', 1)
if (grants.length !== 1 || grants[0].qos !== 1) throw new Error('unexpected QoS1 SUBACK')
const liveMessage = live.nextMessage('m3/mqttjs/live')
await publish(publisher.client, 'm3/mqttjs/live', 'qos1-live')
if ((await liveMessage).payload !== 'qos1-live') throw new Error('QoS1 live payload mismatch')

const persistent = await connectClient('m3-mqttjs-persistent', { clean: false })
if (persistent.connack.sessionPresent) throw new Error('new persistent session must not be present')
await subscribe(persistent.client, 'm3/mqttjs/offline', 1)
await end(persistent.client)
await publish(publisher.client, 'm3/mqttjs/offline', 'offline-qos1')
const resumed = await connectClient('m3-mqttjs-persistent', { clean: false })
if (!resumed.connack.sessionPresent) throw new Error('persistent reconnect must set sessionPresent')
const offline = await resumed.nextMessage('m3/mqttjs/offline')
if (offline.payload !== 'offline-qos1' || offline.packet.qos !== 1) {
  throw new Error('offline persistent QoS1 delivery mismatch')
}

const willObserver = await connectClient('m3-mqttjs-will-observer')
await subscribe(willObserver.client, 'm3/mqttjs/will', 1)
const willMessage = willObserver.nextMessage('m3/mqttjs/will')
const doomed = await connectClient('m3-mqttjs-doomed', {
  will: { topic: 'm3/mqttjs/will', payload: 'qos1-will', qos: 1, retain: true }
})
...
console.log('MQTT.js M3 QoS 1, persistent session, and Will interoperability passed')
