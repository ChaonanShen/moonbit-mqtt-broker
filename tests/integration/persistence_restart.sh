...
seed_output="$(node tests/integration/persistence_restart.mjs seed "mqtt://127.0.0.1:${port}")"
packet_id="$(sed -n 's/^PERSISTENCE_SEED_PACKET_ID=//p' <<<"${seed_output}")"
test -n "${packet_id}"
wait_for_new_commit "${commit_count}"
stop_broker

start_broker "${port}"
grep -Eq 'snapshot restored version=1-or-2-or-3 sessions=[1-9][0-9]* retained=[1-9][0-9]* bytes=[1-9][0-9]*' "${BROKER_LOG}"
commit_count="$(grep -c 'snapshot committed revision=' "${BROKER_LOG}" || true)"
node tests/integration/persistence_restart.mjs verify "mqtt://127.0.0.1:${port}" "${packet_id}"
wait_for_new_commit "${commit_count}"
stop_broker

start_broker "${port}"
node tests/integration/persistence_restart.mjs verify-cleared "mqtt://127.0.0.1:${port}"
stop_broker
echo 'PERSISTENCE MQTT.js retained, session, offline QoS1, inflight DUP, packet ID, and capacity restart recovery passed'
