...
node tests/integration/m5_stability.mjs quick "mqtt://127.0.0.1:${port}"
for index in $(seq 1 500); do
  mosquitto_pub -h 127.0.0.1 -p "${port}" -i "m5-churn-${index}" -t m5/churn -n
done
...
stop_broker
echo "M5_CHURN_FD=${warm_fd}->${current_fd} RSS_KIB=${peak_rss_kib}"

restart_rounds=5
if [[ "${M5_SOAK:-0}" -eq 1 ]]; then restart_rounds=25; fi
max_snapshot_size=0
for round in $(seq 0 $((restart_rounds - 1))); do
  start_broker "${port}" 1
  if [[ "${round}" -gt 0 ]]; then
    node tests/integration/m5_stability.mjs restart-verify "mqtt://127.0.0.1:${port}" $((round - 1))
  fi
  commit_count="$(grep -c 'snapshot committed revision=' "${BROKER_LOG}" || true)"
  node tests/integration/m5_stability.mjs restart-seed "mqtt://127.0.0.1:${port}" "${round}"
...
start_broker "${port}" 1
node tests/integration/m5_stability.mjs restart-verify "mqtt://127.0.0.1:${port}" $((restart_rounds - 1))
stop_broker
echo "M5_RESTART_ROUNDS=${restart_rounds} MAX_SNAPSHOT_BYTES=${max_snapshot_size}"

if [[ "${M5_SOAK:-0}" -eq 1 ]]; then
  start_broker "${port}"
  node tests/integration/m5_stability.mjs extended "mqtt://127.0.0.1:${port}"
  stop_broker
fi

echo 'M5 bounded workload stability profile passed'
