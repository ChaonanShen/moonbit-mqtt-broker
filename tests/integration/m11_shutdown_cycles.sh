...
for cycle in $(seq 1 20); do
  ...
  node tests/integration/m11_shutdown_cycle.mjs seed "mqtt://127.0.0.1:${port}" "${cycle}"
  test ! -e "${data_dir}/broker.snapshot"
  stop "${log}"
  test -s "${data_dir}/broker.snapshot"
  start "${port}" "${data_dir}" "${log}"
  node tests/integration/m11_shutdown_cycle.mjs verify "mqtt://127.0.0.1:${port}" "${cycle}"
  stop "${log}"
done
echo 'M11_SIGTERM_SNAPSHOT_CYCLES=20'
