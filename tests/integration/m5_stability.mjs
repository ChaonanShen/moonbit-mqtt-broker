...
if (!mode || !url) throw new Error('usage: node m5_stability.mjs quick|restart-seed|restart-verify|extended URL [round]')
...
const rawPingCycle = async index => {
  ...
  socket.write(generate({
    ...
    keepalive: 5, clientId: `m5-concurrent-${index}`
  }))
...
const concurrentClients = async count => {
  ...
  console.log(`M5_CONCURRENT_CLIENTS=${count}`)
}

const qos0Sequence = async count => {
  const subscriber = await connect('m5-stability-qos0-sub')
  const received = []
  subscriber.client.on('message', (_topic, payload) => {
    const sequence = Number(payload.subarray(0, 8).toString())
    received.push(sequence)
  })
  await subscribe(subscriber.client, 'm5/stability/qos0', 0)
  const publisher = await connect('m5-stability-qos0-pub')
  const padding = 'x'.repeat(1016)
  for (let base = 0; base < count; base += 32) {
    const batch = []
    for (let index = base; index < Math.min(base + 32, count); index += 1) {
      batch.push(publish(publisher.client, 'm5/stability/qos0', `${String(index).padStart(8, '0')}${padding}`, { qos: 0 }))
    }
  ...
  console.log(`M5_QOS0_MESSAGES=${count}`)
}

const offlineQos1 = async (sessionCount, perSession) => {
  for (let index = 0; index < sessionCount; index += 1) {
    const session = await connect(`m5-offline-${index}`, { clean: false })
    await subscribe(session.client, `m5/stability/offline/${index}`, 1)
    await end(session.client)
  }
  const publisher = await connect('m5-offline-publisher')
  for (let sequence = 0; sequence < perSession; sequence += 1) {
    await Promise.all(Array.from({ length: sessionCount }, (_, index) =>
      publish(publisher.client, `m5/stability/offline/${index}`, String(sequence), { qos: 1 })
    ))
  }
  ...
  console.log(`M5_OFFLINE_QOS1=${sessionCount}x${perSession}`)
}
...
const slowConsumerIsolation = async () => {
  ...
  slow.write(generate({ cmd: 'connect', protocolId: 'MQTT', protocolVersion: 4, clean: true, keepalive: 5, clientId: 'm5-slow' }))
  await waitUntil(() => packets.some(packet => packet.cmd === 'connack'), 'slow CONNACK')
  slow.write(generate({ cmd: 'subscribe', messageId: 1, subscriptions: [{ topic: 'm5/stability/slow', qos: 0 }] }))
  await waitUntil(() => packets.some(packet => packet.cmd === 'suback'), 'slow SUBACK')
  slow.pause()

  const healthy = await Promise.all(Array.from({ length: 20 }, (_, index) => connect(`m5-healthy-${index}`)))
  ...
  await subscribe(observer.client, 'm5/stability/healthy', 0)
  const publisher = healthy[1].client
  const payload = Buffer.alloc(1024, 0x73)
  for (let index = 0; index < 2500; index += 1) {
    publisher.publish('m5/stability/slow', payload, { qos: 0 })
    if (index % 100 === 0) await publish(publisher, 'm5/stability/healthy', `healthy-${index}`, { qos: 0 })
  }
  ...
  slow.destroy()
  console.log('M5_SLOW_CONSUMER_ISOLATED=1')
}
...
const restartSeed = async round => {
  const persistent = await connect('m5-restart-session', { clean: false })
  if (round > 0 && !persistent.connack.sessionPresent) throw new Error(`restart session absent at round ${round}`)
  await subscribe(persistent.client, 'm5/restart/offline', 1)
  await end(persistent.client)
  const publisher = await connect(`m5-restart-publisher-${round}`)
  await publish(publisher.client, 'm5/restart/retained', `round-${round}`, { qos: 1, retain: true })
  await publish(publisher.client, 'm5/restart/offline', `offline-${round}`, { qos: 1 })
  await end(publisher.client)
  console.log(`M5_RESTART_SEEDED=${round}`)
}

const restartVerify = async round => {
  const retained = await connect(`m5-restart-verifier-${round}`)
  await subscribe(retained.client, 'm5/restart/retained', 1)
  ...
  console.log(`M5_RESTART_VERIFIED=${round}`)
}

const extended = async () => {
  const count = Number(process.env.M5_SOAK_PUBLICATIONS ?? '100000')
  const durationMs = Number(process.env.M5_SOAK_SECONDS ?? '600') * 1000
  const started = Date.now()
  await concurrentClients(100)
  await qos0Sequence(count)
  const clients = await Promise.all(Array.from({ length: 100 }, (_, index) => connect(`m5-soak-${index}`)))
  while (Date.now() - started < durationMs) {
    await Promise.all(clients.slice(0, 20).map((value, index) => publish(value.client, `m5/soak/${index}`, 'pulse', { qos: index % 2 })))
    await delay(1000)
  }
  await Promise.all(clients.map(value => end(value.client)))
  console.log(`M5_EXTENDED_PUBLICATIONS=${count}`)
  console.log(`M5_EXTENDED_SECONDS=${Math.floor((Date.now() - started) / 1000)}`)
}
