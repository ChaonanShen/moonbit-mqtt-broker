...
port="$(free_port)"
start_broker "${port}"
node tests/integration/qos1.mjs "mqtt://127.0.0.1:${port}"

first_log="$(mktemp)"
set +e
timeout 1 mosquitto_sub -h 127.0.0.1 -p "${port}" \
  -i qos1-mosquitto-persistent -c -q 1 -t 'qos1/mosquitto/offline' \
  >"${first_log}" 2>&1
first_status="$?"
set -e
...
mosquitto_pub -h 127.0.0.1 -p "${port}" -i qos1-mosquitto-publisher \
  -q 1 -t 'qos1/mosquitto/offline' -m 'mosquitto-qos1'
resumed="$(timeout 5 mosquitto_sub -h 127.0.0.1 -p "${port}" \
  -i qos1-mosquitto-persistent -c -q 1 -t 'qos1/mosquitto/offline' -C 1)"
test "${resumed}" = 'mosquitto-qos1'
echo 'Mosquitto QOS1 QoS 1 persistent-session interoperability passed'
