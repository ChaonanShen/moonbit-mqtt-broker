...
if (!['seed', 'verify'].includes(mode) || !url || !caseName) {
  throw new Error('usage: node graceful_shutdown.mjs seed|verify mqtt://HOST:PORT CASE')
}

const retainedTopic = `shutdown/${caseName}/retained`
const offlineTopic = `shutdown/${caseName}/offline`
const willTopic = `shutdown/${caseName}/shutdown-will`
const persistentClientId = `shutdown-${caseName}-persistent`
...
const seed = async () => {
  ...
  const publisher = await connect(`shutdown-${caseName}-publisher`)
  ...
  const willClient = await connect(`shutdown-${caseName}-will-source`, {
    will: {
      topic: willTopic,
  ...
  console.log(`SHUTDOWN_SEED_READY=${caseName}`)
  await new Promise(resolve => willClient.client.once('close', resolve))
}

const verify = async () => {
  const verifier = await connect(`shutdown-${caseName}-verifier`)
  ...
  console.log(`SHUTDOWN_VERIFIED=${caseName}`)
}
