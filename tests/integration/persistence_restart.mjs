...
if (!mode || !url) {
  throw new Error('usage: node persistence_restart.mjs seed|verify|verify-cleared mqtt://HOST:PORT [packet-id]')
}
...
const seed = async () => {
  const persistent = await connectClient('persistence-persistent', { clean: false })
  if (persistent.connack.sessionPresent) throw new Error('new persistent session unexpectedly present')
  await subscribe(persistent.client, 'persistence/offline', 1)
  await end(persistent.client)

  const clean = await connectClient('persistence-clean', { clean: true })
  await subscribe(clean.client, 'persistence/clean', 1)
  await end(clean.client)

  for (let index = 0; index < 20; index += 1) {
    const session = await connectClient(`persistence-capacity-${index}`, { clean: false })
    await subscribe(session.client, `persistence/capacity/${index}`, 1)
    await end(session.client)
  }

  const publisher = await connectClient('persistence-seed-publisher')
  await publish(publisher.client, 'persistence/retained', 'retained-v1', { retain: true })
  await publish(publisher.client, 'persistence/offline', 'offline-qos1')
  await publish(publisher.client, 'persistence/offline', 'qos0-not-stored', { qos: 0 })
  for (let index = 0; index < 20; index += 1) {
    await publish(publisher.client, `persistence/capacity/${index}`, `capacity-${index}`)
  }

  const willObserver = await connectClient('persistence-will-observer')
  await subscribe(willObserver.client, 'persistence/will', 1)
  const willMessage = willObserver.nextMessage('persistence/will')
  const doomed = await connectClient('persistence-will-source', {
    will: { topic: 'persistence/will', payload: 'will-retained', qos: 1, retain: true }
  })
  ...
  const raw = await rawConnect('persistence-inflight')
  raw.send({
    cmd: 'subscribe',
    messageId: 1,
    subscriptions: [{ topic: 'persistence/inflight', qos: 1 }]
  })
  await raw.nextPacket(packet => packet.cmd === 'suback' && packet.messageId === 1, 'SUBACK')
  await publish(publisher.client, 'persistence/inflight', 'unacked-qos1')
  ...
  console.log(`PERSISTENCE_SEED_PACKET_ID=${unacked.messageId}`)
}

const verify = async expectedPacketId => {
  const retained = await connectClient('persistence-retained-verifier')
  const retainedMessage = retained.nextMessage('persistence/retained')
  await subscribe(retained.client, 'persistence/retained', 1)
  ...
  const willMessage = retained.nextMessage('persistence/will')
  await subscribe(retained.client, 'persistence/will', 1)
  ...
  const offline = await persistent.nextMessage('persistence/offline')
  if (offline.payload !== 'offline-qos1' || offline.packet.qos !== 1) {
    throw new Error('offline QoS1 did not survive restart')
  }

  const clean = await connectClient('persistence-clean', { clean: false })
  ...
  for (let index = 0; index < 20; index += 1) {
    ...
    const message = await session.nextMessage(`persistence/capacity/${index}`)
  ...
  const publisher = await connectClient('verify-persistence-publisher')
  await publish(publisher.client, 'persistence/inflight', 'next-packet-id')
  ...
  await end(persistent.client)
  const cleaner = await connectClient('persistence-persistent', { clean: true })
  await end(cleaner.client)
  await Promise.all([retained.client, publisher.client].map(end))
  console.log('PERSISTENCE restart recovery verified')
}

const verifyCleared = async () => {
  const client = await connectClient('persistence-persistent', { clean: false })
  if (client.connack.sessionPresent) throw new Error('clean reconnect state resurrected after restart')
  await end(client.client)
  console.log('PERSISTENCE clean-session deletion survived restart')
}
