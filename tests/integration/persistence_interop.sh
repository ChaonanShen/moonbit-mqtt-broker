...
DATA_DIR="$(mktemp -d)"
start_broker "${port}"
mosquitto_pub -h 127.0.0.1 -p "${port}" -q 1 -r -t persistence/mosquitto/retained -m retained
set +e
timeout 1 mosquitto_sub -h 127.0.0.1 -p "${port}" \
  -i persistence-mosquitto-persistent -c -q 1 -t persistence/mosquitto/offline >/dev/null 2>&1
status="$?"
set -e
[[ "${status}" -eq 0 || "${status}" -eq 124 ]]
commit_count="$(grep -c 'snapshot committed revision=' "${BROKER_LOG}" || true)"
mosquitto_pub -h 127.0.0.1 -p "${port}" -q 1 -t persistence/mosquitto/offline -m offline
wait_for_new_commit "${commit_count}"
stop_broker

start_broker "${port}"
retained="$(timeout 5 mosquitto_sub -h 127.0.0.1 -p "${port}" -q 1 -t persistence/mosquitto/retained -C 1)"
test "${retained}" = retained
offline="$(timeout 5 mosquitto_sub -h 127.0.0.1 -p "${port}" \
  -i persistence-mosquitto-persistent -c -q 1 -t persistence/mosquitto/offline -C 1)"
test "${offline}" = offline
stop_broker
echo 'Mosquitto PERSISTENCE retained and persistent-session restart interoperability passed'
