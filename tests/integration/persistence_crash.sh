...
DATA_DIR="$(mktemp -d)"
start_broker "${port}"
mosquitto_pub -h 127.0.0.1 -p "${port}" -q 1 -r -t persistence/crash/value -m committed-a
for _ in $(seq 1 300); do
...
value="$(timeout 5 mosquitto_sub -h 127.0.0.1 -p "${port}" -q 1 -t persistence/crash/value -C 1)"
