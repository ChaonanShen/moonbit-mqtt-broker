...
if (!mode || !url) throw new Error('usage: node tls.mjs functional|restart-seed|restart-verify|concurrency|reject-ca|plaintext|stall URL [CA]')
...
const functional = async () => {
  const subscriber = await connect('tls-tls-subscriber')
  await subscribe(subscriber.client, 'tls/tls/live', 1)
  const liveMessage = waitForMessage(subscriber.client, (_topic, payload) => payload === 'encrypted-qos1')
  const publisher = await connect('tls-tls-publisher')
  await publish(publisher.client, 'tls/tls/live', 'encrypted-qos1', { qos: 1 })
  await liveMessage
  await publish(publisher.client, 'tls/tls/retained', 'encrypted-retained', { qos: 1, retain: true })
  const retained = await connect('tls-tls-retained')
  const retainedMessage = waitForMessage(
    retained.client,
    (topic, payload, packet) => topic === 'tls/tls/retained' && payload === 'encrypted-retained' && packet.retain
  )
  await subscribe(retained.client, 'tls/tls/retained', 1)
  await retainedMessage
  await Promise.all([subscriber.client, publisher.client, retained.client].map(end))
  console.log('TLS_FUNCTIONAL=1')
}

const concurrency = async () => {
  const clients = await Promise.all(Array.from({ length: 100 }, (_, index) => connect(`tls-tls-concurrent-${index}`)))
  await Promise.all(clients.map((entry, index) => publish(entry.client, `tls/tls/concurrent/${index}`, 'ok', { qos: 1 })))
  await Promise.all(clients.map(entry => end(entry.client)))
  console.log(`TLS_CONCURRENT=${clients.length}`)
}

const restartSeed = async () => {
  const session = await connect('tls-tls-persistent', { clean: false })
  await subscribe(session.client, 'tls/tls/offline', 1)
  await end(session.client)
  const publisher = await connect('tls-tls-restart-publisher')
  await publish(publisher.client, 'tls/tls/offline', 'encrypted-offline', { qos: 1 })
  await publish(publisher.client, 'tls/tls/restart-retained', 'encrypted-restart-retained', { qos: 1, retain: true })
  await end(publisher.client)
  console.log('TLS_RESTART_SEEDED=1')
}
...
const restartVerify = async () => {
  ...
  const retained = await connect('tls-tls-restart-verifier')
  const replay = waitForMessage(
    retained.client,
    (topic, payload, packet) => topic === 'tls/tls/restart-retained' && payload === 'encrypted-restart-retained' && packet.retain
  )
  await subscribe(retained.client, 'tls/tls/restart-retained', 1)
  await replay
  await Promise.all([session.client, retained.client].map(end))
  console.log('TLS_RESTART_VERIFIED=1')
}

const rejectCa = async () => {
  try {
    const value = await connect('tls-wrong-ca')
  ...
  console.log('TLS_WRONG_CA_REJECTED=1')
}
...
const plaintext = async () => {
  ...
  console.log('TLS_PLAINTEXT_REJECTED=1')
}
...
const stall = async () => {
  ...
  await delay(20)
  console.log('TLS_HANDSHAKE_TIMEOUT=1')
}
