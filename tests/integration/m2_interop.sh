...
port="$(free_port)"
start_broker "${port}"
node tests/integration/m2_qos0.mjs "mqtt://127.0.0.1:${port}"

mosquitto_output="$(mktemp)"
mosquitto_sub -h 127.0.0.1 -p "${port}" -i m2-mosquitto-sub \
  -t 'm2/mosquitto/live' -C 1 >"${mosquitto_output}" &
SUB_PID="$!"
sleep 0.1
mosquitto_pub -h 127.0.0.1 -p "${port}" -i m2-mosquitto-pub \
  -t 'm2/mosquitto/live' -m 'mosquitto-live'
wait "${SUB_PID}"
SUB_PID=""
grep -qx 'mosquitto-live' "${mosquitto_output}"

mosquitto_pub -h 127.0.0.1 -p "${port}" -i m2-mosquitto-retain-pub \
  -t 'm2/mosquitto/retained' -m 'mosquitto-retained' -r
retained="$(timeout 5 mosquitto_sub -h 127.0.0.1 -p "${port}" \
  -i m2-mosquitto-retain-sub -t 'm2/mosquitto/retained' -C 1)"
test "${retained}" = 'mosquitto-retained'
rm -f "${mosquitto_output}"
echo 'Mosquitto M2 QoS 0 and retained interoperability passed'
