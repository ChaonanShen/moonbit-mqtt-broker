...
start_broker never
node tests/integration/m9_session_expiry.mjs never "mqtt://127.0.0.1:${port}"
stop_broker

rm -rf "${data_dir}"
start_broker 250ms
node tests/integration/m9_session_expiry.mjs timing "mqtt://127.0.0.1:${port}"
node tests/integration/m9_session_expiry.mjs seed "mqtt://127.0.0.1:${port}"
for _ in $(seq 1 200); do
  grep -qF 'persistent Session expired client_id=m9-persisted-expiry' "${broker_log}" && break
  sleep 0.025
done
grep -qF 'persistent Session expired client_id=m9-persisted-expiry' "${broker_log}"
stop_broker
test -s "${data_dir}/broker.snapshot"

start_broker 250ms
node tests/integration/m9_session_expiry.mjs verify-absent "mqtt://127.0.0.1:${port}"
stop_broker
...
echo 'M9 Session expiry timing, reset, default, and restart deletion passed'
