...
if (!mode || !url || !caPath) throw new Error('usage: node security.mjs exercise|seed|verify URL CA')
...
const exercise = async () => {
  await rejected('security-missing', undefined, undefined, 5)
  await rejected('security-wrong', 'alice', 'wrong-secret', 4)
  await rejected('security-unknown', 'unknown', 'alice-secret', 4)
  await rejected('security-will-denied', 'alice', 'alice-secret', 5, {
    will: { topic: 'denied/will', payload: 'must-not-fire', qos: 1, retain: true }
  })

  const alice = await connect('security-alice', 'alice', 'alice-secret')
  ...
  const bob = await connect('security-bob-observer', 'bob', 'bob-secret')
  ...
  const bobReplay = await connect('security-bob-replay', 'bob', 'bob-secret')
  ...
  console.log('SECURITY_AUTH_ACL_EXERCISE=1')
}

const seed = async () => {
  const session = await connect('security-owned-session', 'alice', 'alice-secret', { clean: false })
  await subscribe(session.client, [{ topic: 'allowed/offline', qos: 1 }])
  await end(session.client)
  await rejected('security-owned-session', 'bob', 'bob-secret', 5, { clean: false })
  const publisher = await connect('security-seed-publisher', 'alice', 'alice-secret')
  await publish(publisher.client, 'allowed/offline', 'owner-offline', { qos: 1 })
  await end(publisher.client)
  console.log('SECURITY_OWNER_SEEDED=1')
}

const verify = async () => {
  await rejected('security-owned-session', 'bob', 'bob-secret', 5, { clean: false })
  await rejected('security-owned-session', 'bob', 'bob-secret', 5, { clean: true })
  const session = await connect('security-owned-session', 'alice', 'alice-secret', { clean: false })
  ...
  await end(session.client)
  console.log('SECURITY_OWNER_RESTART_VERIFIED=1')
}
