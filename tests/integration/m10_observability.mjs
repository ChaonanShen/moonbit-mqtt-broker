...
if (!url || !caPath || !wrongCaPath) throw new Error('usage: node m10_observability.mjs URL CA WRONG_CA')
...
const expectRejected = async (options, description) => {
  try {
    const result = await connect(`m10-rejected-${description}`, options)
...
const observer = await connect('m10-observer', { clean: false })
...
const ordinary = await connect('m10-ordinary', {
  username: 'ordinary',
  password: 'ordinary-secret'
})
await subscribe(ordinary.client, '#')
await publish(observer.client, 'allowed/application', 'M10_PAYLOAD_SECRET_MARKER', { qos: 1 })
await publish(observer.client, '$SYS/broker/forged', 'M10_FORGED_SECRET_MARKER', { qos: 1, retain: true })
...
console.log('M10_METRICS_VERIFIED=1')
