...
run_matrix() {
  ...
  node tests/integration/m5_protocol_matrix.mjs "mqtt://127.0.0.1:${port}" common "${label}" \
    | sed -n 's/^M5_MATRIX_RESULT=//p' >"${output}"
  test -s "${output}"
}
...
echo 'M5 MoonBit, Mosquitto, and Aedes normalized MQTT 3.1.1 behavior agrees'
