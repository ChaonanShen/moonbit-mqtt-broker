...
try {
  ...
  const disabledReload = await request('POST', '/v1/config/reload', configToken)
  assert.equal(disabledReload.status, 503, disabledReload.text)
  assert.equal(json(disabledReload).error.code, 'reload_disabled')
  const disabledReloadWithBody = await request('POST', '/v1/config/reload', configToken, {}, '{}')
  assert.equal(disabledReloadWithBody.status, 503, disabledReloadWithBody.text)
  assert.equal(json(disabledReloadWithBody).error.code, 'reload_disabled')
