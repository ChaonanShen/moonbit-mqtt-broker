...
node tests/integration/tls.mjs functional "${TLS_URL}" "${WORK_DIR}/server.crt"

mosquitto_output="${WORK_DIR}/mosquitto.out"
mosquitto_sub -h localhost -p "${port}" --cafile "${WORK_DIR}/server.crt" \
  -i tls-mosquitto-sub -t tls/mosquitto/live -q 1 -C 1 >"${mosquitto_output}" &
SUB_PID="$!"
sleep 0.15
mosquitto_pub -h localhost -p "${port}" --cafile "${WORK_DIR}/server.crt" \
  -i tls-mosquitto-pub -t tls/mosquitto/live -q 1 -m encrypted-mosquitto
wait "${SUB_PID}"
SUB_PID=""
grep -qxF 'encrypted-mosquitto' "${mosquitto_output}"

node tests/integration/tls.mjs reject-ca "${TLS_URL}" "${WORK_DIR}/wrong.crt"
node tests/integration/tls.mjs plaintext "mqtt://127.0.0.1:${port}"
node tests/integration/tls.mjs stall "mqtt://127.0.0.1:${port}"
node tests/integration/tls.mjs concurrency "${TLS_URL}" "${WORK_DIR}/server.crt"
node tests/integration/tls.mjs functional "${TLS_URL}" "${WORK_DIR}/server.crt"
node tests/integration/tls.mjs restart-seed "${TLS_URL}" "${WORK_DIR}/server.crt"
...
start_tls_broker
node tests/integration/tls.mjs restart-verify "${TLS_URL}" "${WORK_DIR}/server.crt"
kill -s TERM -- "-${BROKER_PID}"
wait "${BROKER_PID}"
BROKER_PID=""
echo 'TLS TLS listener, validation, timeout, rejection, concurrency, and interoperability passed'
