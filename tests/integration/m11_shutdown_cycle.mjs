...
if (!mode || !url || !cycle) throw new Error('usage: node m11_shutdown_cycle.mjs seed|verify URL CYCLE')
const clientId = `m11-shutdown-${cycle}`
const topic = `m11/shutdown/${cycle}`
...
if (mode === 'seed') {
  ...
  const publisher = mqtt.connect(url, { clientId: `m11-publisher-${cycle}`, protocolVersion: 4, reconnectPeriod: 0 })
