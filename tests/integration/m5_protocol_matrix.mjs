...
if (!url || !['common', 'full'].includes(profile)) {
  throw new Error('usage: node m5_protocol_matrix.mjs mqtt://HOST:PORT common|full [label]')
}
...
const commonMatrix = async () => {
  const subscriber = await connect(`m5-${label}-common-sub`)
  result.cleanSessionPresent = subscriber.connack.sessionPresent
  const grants = await subscribe(subscriber.client, [
    { topic: 'm5/common/+/value', qos: 0 },
    { topic: 'm5/common/#', qos: 1 },
    { topic: 'm5/empty/+/tail', qos: 1 }
  ])
  result.suback = grants.map(grant => grant.qos)

  const publisher = await connect(`m5-${label}-common-pub`)
  const overlap = subscriber.next(message => message.topic === 'm5/common/a/value', 'overlap')
  await publish(publisher.client, 'm5/common/a/value', 'overlap', { qos: 1 })
  ...
  const empty = subscriber.next(message => message.topic === 'm5/empty//tail', 'empty level')
  await publish(publisher.client, 'm5/empty//tail', 'empty-level', { qos: 1 })
  result.emptyLevel = (await empty).payload

  await publish(publisher.client, 'm5/common/retained', 'retained-value', { qos: 1, retain: true })
  const retainedClient = await connect(`m5-${label}-retained`)
  const retained = retainedClient.next(message => message.topic === 'm5/common/retained', 'retained replay')
  await subscribe(retainedClient.client, 'm5/common/retained')
  const retainedMessage = await retained
  result.retained = { payload: retainedMessage.payload, retain: retainedMessage.retain }
  await publish(publisher.client, 'm5/common/retained', '', { qos: 1, retain: true })
  await end(retainedClient.client)

  await unsubscribe(subscriber.client, 'm5/common/#')
...
const fullMatrix = async () => {
  const persistent = await connect('m5-full-persistent', { clean: false })
  if (persistent.connack.sessionPresent) throw new Error('new persistent session is present')
  await subscribe(persistent.client, 'm5/full/offline')
  await end(persistent.client)
  const publisher = await connect('m5-full-publisher')
  for (let index = 0; index < 3; index += 1) {
    await publish(publisher.client, 'm5/full/offline', `offline-${index}`, { qos: 1 })
  }
  const resumed = await connect('m5-full-persistent', { clean: false })
  if (!resumed.connack.sessionPresent) throw new Error('persistent session is absent')
  const offline = []
  for (let index = 0; index < 3; index += 1) {
    offline.push((await resumed.next(message => message.topic === 'm5/full/offline', `offline ${index}`)).payload)
  }
  ...
  const observer = await connect('m5-full-will-observer')
  await subscribe(observer.client, 'm5/full/will')
  const forcedWill = observer.next(message => message.topic === 'm5/full/will', 'forced Will')
  const doomed = await connect('m5-full-will-forced', {
    will: { topic: 'm5/full/will', payload: 'forced', qos: 1, retain: false }
  })
  doomed.client.end(true)
  result.forcedWill = (await forcedWill).payload
  const graceful = await connect('m5-full-will-graceful', {
    will: { topic: 'm5/full/will', payload: 'must-not-fire', qos: 1, retain: false }
  })
  ...
  const duplicate = await rawConnect('m5-full-duplicate-connect')
...
console.log(`M5_MATRIX_RESULT=${JSON.stringify(result)}`)
