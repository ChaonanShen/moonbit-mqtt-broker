...
node tests/integration/m8_security.mjs exercise "${URL}" "${WORK_DIR}/server.crt"

mosquitto_output="${WORK_DIR}/mosquitto.out"
mosquitto_sub -h localhost -p "${port}" --cafile "${WORK_DIR}/server.crt" \
  -u alice -P alice-secret -i m8-mosquitto-sub -t allowed/mosquitto -q 1 -C 1 \
  >"${mosquitto_output}" &
SUB_PID="$!"
sleep 0.15
mosquitto_pub -h localhost -p "${port}" --cafile "${WORK_DIR}/server.crt" \
  -u alice -P alice-secret -i m8-mosquitto-pub -t allowed/mosquitto -q 1 \
  -m authenticated-mosquitto
wait "${SUB_PID}"
SUB_PID=""
grep -qxF 'authenticated-mosquitto' "${mosquitto_output}"

node tests/integration/m8_security.mjs seed "${URL}" "${WORK_DIR}/server.crt"
...
start_broker
node tests/integration/m8_security.mjs verify "${URL}" "${WORK_DIR}/server.crt"
kill -s TERM -- "-${BROKER_PID}"
wait "${BROKER_PID}"
BROKER_PID=""
echo 'M8 Argon2id, ACL, Principal ownership, Snapshot V2, TLS, and restart passed'
