...
node tests/integration/observability.mjs \
...
! grep -qF 'OBSERVABILITY_PAYLOAD_SECRET_MARKER' "${broker_log}"
! grep -qF 'OBSERVABILITY_FORGED_SECRET_MARKER' "${broker_log}"
...
echo 'OBSERVABILITY system metrics, JSON logs, levels, ACL isolation, and redaction passed'
