...
请使用 TLS 并验证 Broker 证书。热更新可轮换既有 MQTT TLS/WSS 监听器的
服务器证书材料，但不支持 mTLS 或管理令牌在线轮换。

## 在线安全撤权

安全热更新成功时先发布新授权 gate，再有界清理旧状态。被删除的用户或
被关闭的匿名主体不能恢复会话，其所属会话会被删除。密码改变会关闭持有
旧凭据的空闲连接；用户仍存在且有权限时，可用新密码恢复原持久会话。
仅修改 ACL 不强制未改密码的用户重连，但新的发布、订阅、Will、
待发消息提升和出站 MQTT 消息都会按当前 ACL 判定。失效订阅与排队
