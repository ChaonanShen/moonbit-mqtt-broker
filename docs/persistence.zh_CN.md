...
启用热更新后，off 模式只有当前内存代际。snapshot 模式会在接纳连接前
使用当前安全策略复核恢复的会话，但快照仍遵守下文的异步持久化边界。
崩溃可能恢复清理前的数据，因此启动时要按当前 bundle 完成复核后才
开放接入。

strict 模式先将版本化 policy transition 提交 WAL，再发布配置代际；
随后持久提交会话驱逐、订阅与待发消息清理，并标记复核完成。恢复会重放
policy 记录，在绑定监听器前完成未结束的清理。当前 bundle 的安全指纹
必须与已提交策略一致，否则启动报 `PolicySourceMismatch`。若替换来源
文件后激活失败，重启前应恢复与旧提交匹配的 bundle。提交 policy 能力后，
不支持该能力的旧读取器不能打开该 WAL 目录。应同时备份数据目录和匹配的
配置 bundle。回退策略也是一次新热更新，不会复活已删除的会话或消息。
